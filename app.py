...
# Password Protection - UPDATED VERSION
def authenticate():
    if 'authenticated' not in st.session_state:
        st.session_state.authenticated = False
    
    if not st.session_state.authenticated:
        # Use columns for centered layout without page config conflict
...
def main():
    ...
    # Enhanced Market Scanner - FIXED SYNTAX ERROR
    st.sidebar.header("⚡ QUICK ACTIONS")
    if st.sidebar.button("🔍 SCAN ALL STOCKS"):
        with st.spinner("Scanning all stocks..."):  # FIXED: Removed extra )
            results = []
